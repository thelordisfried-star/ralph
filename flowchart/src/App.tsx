...
function App() {
  ...
  // ⚡ Bolt Optimization: Replace O(N) node/edge recreation with functional state setters to mutate only styles, preserving referential stability.
  const updateVisibility = useCallback((newCount: number) => {
    ...
    setNodes((nds) => nds.map((node) => {
      ...
      if (node.type === 'custom') {
        ...
        isVisible = index < newCount;
      } else if (node.type === 'note') {
        const noteConfig = notes.find(n => n.id === node.id);
        isVisible = noteConfig ? newCount >= noteConfig.appearsWithStep : false;
      }

      const expectedPointerEvents: 'auto' | 'none' = isVisible ? 'auto' : 'none';
      if (node.style?.opacity === (isVisible ? 1 : 0) && node.style?.pointerEvents === expectedPointerEvents) {
        return node;
      }
      return {
        ...node,
        style: { ...node.style, opacity: isVisible ? 1 : 0, pointerEvents: expectedPointerEvents },
      };
    }));
    ...
    setEdges((eds) => eds.map((edge) => {
      ...
      return {
        ...
        style: { ...edge.style, opacity: isVisible ? 1 : 0, stroke: isVisible ? '#222' : 'transparent' },
      };
    }));
  }, [setNodes, setEdges]);
