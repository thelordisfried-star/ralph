...
const noteAppearsWithStepMap = new Map(notes.map(n => [n.id, n.appearsWithStep]));
const edgeLabelMap = new Map(edgeConnections.map(c => [`e${c.source}-${c.target}`, c.label]));

const updateVisibility = (visibleCount: number, nodes: Node[], edges: Edge[]) => {
  const nextNodes = nodes.map(node => {
    let visible = false;
    if (node.type === 'custom') {
      const stepIndex = stepIndexMap.get(node.id) ?? -1;
      visible = stepIndex !== -1 && stepIndex < visibleCount;
    } else if (node.type === 'note') {
      const appearsWithStep = noteAppearsWithStepMap.get(node.id) ?? Infinity;
      visible = visibleCount >= appearsWithStep;
    }
  ...
  const nextEdges = edges.map(edge => {
    ...
    const visible = sourceIndex !== -1 && targetIndex !== -1 && sourceIndex < visibleCount && targetIndex < visibleCount;

    const targetOpacity = visible ? 1 : 0;

    if (edge.style?.opacity === targetOpacity && edge.animated === visible) {
      return edge;
    }
...
function App() {
  ...
  const initialEdges = useMemo<Edge[]>(() => edgeConnections.map((conn, index) => createEdge(conn, index < 0)), []);
  ...
  const handleNext = useCallback(() => {
    if (visibleCount < allSteps.length) {
      ...
      // Update nodes and edges independently but in the same render batch
      setNodes(nds => updateVisibility(newCount, nds, []).nextNodes);
      setEdges(eds => updateVisibility(newCount, [], eds).nextEdges);
    }
