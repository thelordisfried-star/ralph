    ...
    if (visibleCount < allSteps.length) {
      ...
      setNodes((nds) => nds.map((node) => {
        ...
        if (node.type === 'custom') {
          ...
          const noteAppearsWithStep = noteAppearsMap.get(node.id) ?? 999;
          isVisible = newCount >= noteAppearsWithStep;
        }

        const newOpacity = isVisible ? 1 : 0;
        if (node.style?.opacity === newOpacity) {
          return node;
        }
      ...
      setEdges((eds) => eds.map((edge) => {
        const sourceIndex = stepIndexMap.get(edge.source as string) ?? -1;
        const targetIndex = stepIndexMap.get(edge.target as string) ?? -1;
        ...
        return {
          ...edge,
          label: isVisible ? edgeLabelMap.get(edge.id) as string | undefined : undefined,
          animated: isVisible,
          style: {
            ...edge.style,
            opacity: newOpacity,
