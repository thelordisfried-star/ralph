...
function createNode(step: typeof allSteps[0], visible: boolean, position?: { x: number; y: number }): Node {
  return {
    ...
    data: stepDataMap.get(step.id)!, // Use stable object
    style: {
      width: nodeWidth,
      height: nodeHeight,
      opacity: visible ? 1 : 0,
...
function createNoteNode(note: typeof notes[0], visible: boolean, position?: { x: number; y: number }): Node {
  return {
    ...
    data: noteDataMap.get(note.id)!, // Use stable object
    style: {
      opacity: visible ? 1 : 0,
      transition: 'opacity 0.5s ease-in-out',
      pointerEvents: visible ? 'auto' : 'none',
...
const getNodes = (count: number, currentPositions: typeof positions) => {
...
function App() {
  ...
  // Use static positions for initialization to avoid ref access during render
