...
function App() {
  ...
  const initialNodes = useMemo(() => {
    ...
    const noteNodes = notes.map(note => createNoteNode(note as typeof notes[0], 1 >= note.appearsWithStep, positions[note.id]));
    return [...stepNodes, ...noteNodes];
  }, []);
  const initialEdges = useMemo(() => edgeConnections.map(conn => createEdge(conn, false)), []);

  const [nodes, setNodes] = useNodesState(initialNodes);
  const [edges, setEdges] = useEdgesState(initialEdges);

  const updateVisibility = useCallback((visibleStepCount: number) => {
    setNodes((nds) => nds.map((node) => {
      ...
      if (node.type === 'custom') {
        ...
        isVisible = visibleStepCount >= appearsWithStep;
      }
      return {
        ...node,
        style: {
    ...
    setEdges((eds) => eds.map((edge) => {
      ...
      const isVisible = sourceIndex !== -1 && targetIndex !== -1 && sourceIndex < visibleStepCount && targetIndex < visibleStepCount;
      return {
        ...edge,
        ...
        style: {
          ...edge.style,
          stroke: isVisible ? '#222' : 'transparent',
          opacity: isVisible ? 1 : 0,
        },
      };
    }));
