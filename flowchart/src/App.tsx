...
function App() {
  ...
  // ⚡ Bolt Optimization: Use functional setters to surgically update only opacity, pointerEvents, animated, and label properties
  // on existing nodes/edges to prevent expensive array recreation and referential identity loss on Next/Prev clicks.
  const updateVisibility = useCallback((newCount: number) => {
    setVisibleCount(newCount);
    setNodes((nds) => nds.map((node) => {
      const isVisible = node.type === 'note' ? newCount >= (noteAppearsMap.get(node.id) ?? Infinity) : (stepIndexMap.get(node.id) ?? Infinity) < newCount;
      const opacity = isVisible ? 1 : 0;
      const pointerEvents: 'auto' | 'none' = isVisible ? 'auto' : 'none';
      if (node.style?.opacity === opacity) return node;
      return { ...node, style: { ...node.style, opacity, pointerEvents } };
    }));
    setEdges((eds) => eds.map((edge) => {
      ...
      if (edge.animated === isVisible) return edge;
      return { ...edge, animated: isVisible, label: isVisible ? (edge.data?.originalLabel as string | undefined) : undefined, style: { ...edge.style, opacity: isVisible ? 1 : 0 } };
    }));
  }, [setNodes, setEdges]);
