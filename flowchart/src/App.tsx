...
// Optimization: Pre-compute static data to ensure referential stability
...
// Optimization: Static style objects to avoid recreation on every render
// Step nodes have fixed dimensions
...
const invisibleStepStyle: CSSProperties = {
  width: nodeWidth,
  height: nodeHeight,
  opacity: 0,
  transition: 'opacity 0.5s ease-in-out',
...
// Note nodes size naturally based on content
...
function createNode(step: typeof allSteps[0], visible: boolean, position?: { x: number; y: number }): Node {
  return {
    ...
    style: visible ? visibleStepStyle : invisibleStepStyle,
  };
}
...
function createEdge(conn: typeof edgeConnections[0], visible: boolean): Edge {
  return {
    ...
    animated: visible,
    style: visible ? visibleEdgeStyle : invisibleEdgeStyle,
    labelStyle: {
      fill: '#222',
      fontWeight: 600,
      fontSize: 14,
...
function createNoteNode(note: typeof notes[0], visible: boolean, position?: { x: number; y: number }): Node {
  return {
    ...
    style: visible ? visibleNoteStyle : invisibleNoteStyle,
    draggable: true,
    selectable: false,
    connectable: false,
  };
...
function App() {
  ...
  // Pass currentPositions to avoid accessing ref during render
