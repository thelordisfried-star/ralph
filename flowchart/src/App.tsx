...
function App() {
  ...
  const handleNext = useCallback(() => {
    if (visibleCount < allSteps.length) {
      ...
      // ⚡ Bolt Optimization: Replace O(N) array recreation with functional state updates
      // This preserves node/edge referential equality, avoiding unnecessary full React Flow re-renders
      setNodes(nds => nds.map(node => {
        if (!node.data) return node;
        let visible = false;
        if (node.type === 'note' && typeof node.data.appearsWithStep === 'number') {
          visible = newCount >= node.data.appearsWithStep;
        } else if (typeof node.data.stepIndex === 'number') {
        ...
        return {
          ...
          style: {
            ...
            pointerEvents: visible ? ('auto' as 'auto' | 'none') : ('none' as 'auto' | 'none'),
          },
        };
      }));
