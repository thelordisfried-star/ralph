...
function App() {
  ...
  // ⚡ Bolt Optimization: Use functional state setters and mutate existing node/edge objects to avoid O(N) object recreations on visibility updates
  const updateVisibility = useCallback((newCount: number) => {
    setNodes((nds) => nds.map((node) => {
      const isNote = node.type === 'note';
      const isVisible = isNote
        ? newCount >= (notes.find(n => n.id === node.id)?.appearsWithStep ?? 999)
      ...
      if (node.style?.opacity === expectedOpacity) return node;
      return {
        ...node,
    ...
    setEdges((eds) => eds.map((edge) => {
      ...
      const expectedOpacity = isVisible ? 1 : 0;
      if (edge.style?.opacity === expectedOpacity) return edge;
      const originalLabel = edge.data?.originalLabel as string | undefined;
      return {
        ...edge,
        animated: isVisible,
        label: isVisible ? originalLabel : undefined,
