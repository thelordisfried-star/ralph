...
function App() {
  ...
  // ⚡ Bolt Optimization: Use functional state setters and check for actual changes
  // before mutating. This avoids O(N) array/object recreations on every step change
  // and preserves referential stability for unmodified elements.
  const updateVisibility = useCallback((newCount: number) => {
    ...
    setEdges((eds) => eds.map((edge) => {
      ...
      const expectedOpacity = isVisible ? 1 : 0;
      const expectedLabel = isVisible ? edge.data?.originalLabel : undefined;

      if (edge.style?.opacity === expectedOpacity && edge.label === expectedLabel && edge.animated === isVisible) {
        return edge;
      }
      ...
      return {
        ...edge,
        animated: isVisible,
        label: expectedLabel as string | undefined,
        style: {
          ...edge.style,
          opacity: expectedOpacity,
        },
