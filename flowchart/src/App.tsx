...
function App() {
  ...
  const initialNodes = useMemo<Node[]>(() => [
    ...allSteps.map((step, index) => createNode(step, index < 1)),
    ...notes.map(note => createNoteNode(note, 1 >= note.appearsWithStep))
  ], []);
    ...
    if (visibleCount < allSteps.length) {
      ...
      setEdges(eds => eds.map(edge => {
        ...
        return {
          ...edge,
          animated: visible,
          label: visible ? edgeConnections.find(c => `e${c.source}-${c.target}` === edge.id)?.label : undefined,
