...
function App() {
  ...
  // ⚡ Bolt Optimization: Selective visibility update using functional state setters
  const updateVisibility = useCallback((newCount: number) => {
    setNodes(nds => nds.map(node => {
      const isNote = node.type === 'note';
      const visible = isNote ? newCount >= (node.data?.appearsWithStep as number) : (node.data?.stepIndex as number) < newCount;
