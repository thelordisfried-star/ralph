...
function createEdge(conn: typeof edgeConnections[0], visible: boolean, sourceIndex: number, targetIndex: number): Edge {
  return {
    ...
    animated: visible,
    data: { originalLabel: conn.label, sourceIndex, targetIndex },
    style: {
      stroke: '#222',
      strokeWidth: 2,
      opacity: visible ? 1 : 0,
...
function App() {
  ...
  const initialEdges = useMemo<Edge[]>(() => edgeConnections.map((conn) =>
    createEdge(conn, false, stepIndexMap.get(conn.source) ?? -1, stepIndexMap.get(conn.target) ?? -1)
  ), []);
  ...
  const handleNext = useCallback(() => {
    if (visibleCount < allSteps.length) {
      ...
      setNodes((nds) => nds.map(node => {
        ...
        } else if (node.type === 'custom' && typeof node.data.stepIndex === 'number') {
        ...
        if (node.style?.opacity === newOpacity && node.style?.pointerEvents === newPointerEvents) {
          return node; // Preserves referential equality
        }
      ...
      setEdges((eds) => eds.map(edge => {
        ...
        if (edge.animated === isVisible) {
          return edge; // Preserves referential equality
        }
