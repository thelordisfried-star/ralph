...
function createEdge(conn: typeof edgeConnections[0], visible: boolean): Edge {
  return {
    ...
    animated: visible,
    data: { sourceIndex: stepIndexMap.get(conn.source), targetIndex: stepIndexMap.get(conn.target), originalLabel: conn.label },
    style: {
      stroke: '#222',
      strokeWidth: 2,
      opacity: visible ? 1 : 0,
...
setNodes((nds) => nds.map((node) => {
  ...
  if (node.type === 'custom') {
    ...
    isVisible = stepIndex !== undefined ? stepIndex < newCount : false;
  ...
  const pointerEvents = isVisible ? ('auto' as 'auto' | 'none') : ('none' as 'auto' | 'none');
...
setEdges((eds) => eds.map((edge) => {
  ...
  const isVisible = sourceIndex !== undefined && targetIndex !== undefined
    ? sourceIndex < newCount && targetIndex < newCount
    : false;
  const opacity = isVisible ? 1 : 0;
