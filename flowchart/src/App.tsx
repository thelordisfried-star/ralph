...
function App() {
  ...
  const initialEdges = useMemo<Edge[]>(() => edgeConnections.map((conn) => createEdge(conn, false)), []);
    ...
    if (visibleCount < allSteps.length) {
      ...
      setNodes((nds) =>
        nds.map((node) => {
          ...
          return {
            ...node,
            style: { ...node.style, opacity: targetOpacity, pointerEvents: targetPointerEvents as 'auto' | 'none' },
          };
        })
      );
