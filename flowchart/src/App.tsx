...
function createNode(step: typeof allSteps[0], index: number, visible: boolean): Node {
  return {
    id: step.id,
    type: 'custom',
...
function createNoteNode(note: typeof notes[0], visible: boolean): Node {
  return {
    id: note.id,
    type: 'note',
...
const getNodes = (count: number) => {
  const stepNodes = allSteps.map((step, index) =>
    createNode(step, index, index < count)
  );
  const noteNodes = notes.map(note => {
    const noteVisible = count >= note.appearsWithStep;
    return createNoteNode(note, noteVisible);
  });
...
function App() {
  ...
  const initialNodes = useMemo<Node[]>(() => getNodes(1), []);
  const initialEdges = useMemo<Edge[]>(() => edgeConnections.map((conn, index) =>
  ...
  const updateVisibility = useCallback((newCount: number) => {
    ...
    // ⚡ Bolt Optimization: Use functional state updates to preserve internal state and avoid O(N) recreations
    setNodes(nds => nds.map(node => {
      if (!node.data) return node;
      const isVisible = node.type === 'note'
        ? newCount >= (node.data.appearsWithStep as number)
        : (node.data.stepIndex as number) < newCount;

      const opacity = isVisible ? 1 : 0;
      const pointerEvents = isVisible ? 'auto' : 'none';
      if (node.style?.opacity === opacity && node.style?.pointerEvents === pointerEvents) return node;
      return { ...node, style: { ...node.style, opacity, pointerEvents: pointerEvents as 'auto' | 'none' } };
    }));
    ...
    setEdges(eds => eds.map(edge => {
      ...
      return { ...edge, label: isVisible ? (edge.data.originalLabel as string | undefined) : undefined, animated: isVisible, style: { ...edge.style, opacity } };
    }));
  }, [setNodes, setEdges]);

  const handleNext = useCallback(() => {
    if (visibleCount < allSteps.length) updateVisibility(visibleCount + 1);
  }, [visibleCount, updateVisibility]);

  const handlePrev = useCallback(() => {
    if (visibleCount > 1) updateVisibility(visibleCount - 1);
  }, [visibleCount, updateVisibility]);
