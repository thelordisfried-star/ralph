...
// ⚡ Bolt Optimization: Wrap in React.memo to prevent global re-renders on graph state updates
...
// ⚡ Bolt Optimization: Externalize static data and styles to maintain referential stability for React.memo
...
function createNode(step: typeof allSteps[0], visible: boolean, position?: { x: number; y: number }): Node {
  return {
    ...
    // ⚡ Bolt Optimization: Use stable references for memoized components
...
function App() {
  ...
  const handleReset = useCallback(() => {
    ...
    // ⚡ Bolt Optimization: Reuse memoized initialNodes and initialEdges to avoid O(N) object recreation
