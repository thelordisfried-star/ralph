...
function App() {
  ...
  const updateVisibility = useCallback((newCount: number) => {
    ...
    setNodes((nds) =>
      nds.map((node) => {
        let isVisible = false;
        if (node.type === 'note') {
          isVisible = newCount >= (noteAppearsMap.get(node.id) || 999);
        } else {
      ...
      eds.map((edge) => {
        ...
        if (edge.style?.opacity !== expectedOpacity) {
          return {
            ...edge,
            animated: isVisible,
            label: isVisible ? edge.data?.originalLabel : undefined,
            style: {
              ...edge.style,
              opacity: expectedOpacity,
            } as import('react').CSSProperties,
  ...
  // ⚡ Bolt Optimization: Use functional setters to selectively modify node/edge styles based on visibility,
  // avoiding O(N) recreations and preserving referential stability.
