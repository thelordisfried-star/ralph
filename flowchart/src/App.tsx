...
const edgeLabelMap = new Map(
  edgeConnections.map((c) => [`e${c.source}-${c.target}`, c.label])
);
...
function App() {
  ...
  const updateNodesVisibility = useCallback((count: number) => {
    setNodes((nds) =>
      nds.map((node) => {
        let isVisible = false;
        if (node.type === 'note') {
          isVisible = count >= (noteAppearsMap.get(node.id) ?? 999);
        } else {
          isVisible = (stepIndexMap.get(node.id) ?? 999) < count;
        }
  ...
  const updateEdgesVisibility = useCallback((count: number) => {
    setEdges((eds) =>
      eds.map((edge) => {
        ...
        const opacity = isVisible ? 1 : 0;
        const label = isVisible ? edgeLabelMap.get(edge.id) : undefined;

        if (edge.style?.opacity === opacity && edge.animated === isVisible && edge.label === label) {
          return edge;
        }
