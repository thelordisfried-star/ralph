...
function createEdge(conn: typeof edgeConnections[0], visible: boolean): Edge {
  return {
    ...
    data: { sourceIndex: stepIndexMap.get(conn.source) ?? -1, targetIndex: stepIndexMap.get(conn.target) ?? -1, originalLabel: conn.label },
  ...
  const updateVisibility = useCallback((newCount: number) => {
    setNodes((nds) =>
      nds.map((node) => {
        ...
        if (node.type === 'custom' && typeof node.data.stepIndex === 'number') {
          ...
          visible = node.data.appearsWithStep <= newCount;
        }

        const currentOpacity = visible ? 1 : 0;
        if (node.style?.opacity === currentOpacity) return node;
    ...
    setEdges((eds) =>
      eds.map((edge) => {
        ...
        const sIdx = typeof edge.data.sourceIndex === 'number' ? edge.data.sourceIndex : -1;
        const tIdx = typeof edge.data.targetIndex === 'number' ? edge.data.targetIndex : -1;
        const visible = sIdx < newCount && tIdx < newCount;

        const currentOpacity = visible ? 1 : 0;
        if (edge.style?.opacity === currentOpacity) return edge;
