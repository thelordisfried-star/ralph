...
function App() {
  ...
  // ⚡ Bolt Optimization: Use functional state updates to modify node and edge visibility without O(N) recreations.
  const updateVisibility = useCallback((newCount: number) => {
    ...
    setNodes((nds) => nds.map((node) => {
      ...
      if (node.style?.opacity === opacity) return node; // Referentially stable
    ...
    setEdges((eds) => eds.map((edge) => {
      ...
      const opacity = isVisible ? 1 : 0;
      if (edge.style?.opacity === opacity) return edge; // Referentially stable
