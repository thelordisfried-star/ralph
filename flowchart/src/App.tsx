...
function createNode(step: typeof allSteps[0], visible: boolean, position?: { x: number; y: number }): Node {
  return {
    ...
    data: {
      ...
      phase: step.phase,
      stepIndex: stepIndexMap.get(step.id),
    },
    style: {
      width: nodeWidth,
      height: nodeHeight,
...
function createEdge(conn: typeof edgeConnections[0], visible: boolean): Edge {
  ...
  return {
    ...
    data: {
      ...
      appearsWithStep: Math.max(sourceIndex, targetIndex) + 1,
    },
    style: {
      stroke: '#222',
      strokeWidth: 2,
...
function App() {
  ...
  const initialEdges = useMemo<Edge[]>(() => {
    return edgeConnections.map((conn) => {
      const sourceIndex = stepIndexMap.get(conn.source) ?? -1;
      const targetIndex = stepIndexMap.get(conn.target) ?? -1;
      const appearsWithStep = Math.max(sourceIndex, targetIndex) + 1;
      return createEdge(conn, appearsWithStep <= 1);
    });
  }, []);
  ...
  const updateVisibility = useCallback((newCount: number) => {
    ...
    setNodes((nds) =>
      nds.map((node) => {
        ...
        if (node.type === 'custom' && typeof node.data.stepIndex === 'number') {
          isVisible = node.data.stepIndex < newCount;
        } else if (node.type === 'note' && typeof node.data.appearsWithStep === 'number') {
          isVisible = newCount >= node.data.appearsWithStep;
        } else {
          return node;
        }
    ...
    setEdges((eds) =>
      eds.map((edge) => {
        if (!edge.data || typeof edge.data.appearsWithStep !== 'number') return edge;
        const isVisible = newCount >= edge.data.appearsWithStep;
        const opacity = isVisible ? 1 : 0;
        if (edge.style?.opacity === opacity) return edge;

        return {
          ...edge,
          label: (isVisible ? edge.data.originalLabel : undefined) as string | undefined,
          animated: isVisible,
          style: {
            ...edge.style,
            opacity,
