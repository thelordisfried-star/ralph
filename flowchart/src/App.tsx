...
function createNode(step: typeof allSteps[0], visible: boolean, position?: { x: number; y: number }, index?: number): Node {
  return {
    id: step.id,
    type: 'custom',
  ...
  const updateVisibility = useCallback((newCount: number) => {
    setNodes((nds) =>
      nds.map((node) => {
        let visible = false;
        if (node.type === 'note') {
          visible = newCount >= (node.data?.appearsWithStep as number ?? 0);
        } else {
          visible = (node.data?.stepIndex as number ?? -1) < newCount;
        }
        ...
        if (
          ...
          node.style?.pointerEvents === targetPointerEvents
        ) {
          return node;
        }
    ...
    setEdges((eds) =>
      eds.map((edge) => {
        const sourceIndex = edge.data?.sourceIndex as number ?? -1;
        const targetIndex = edge.data?.targetIndex as number ?? -1;
