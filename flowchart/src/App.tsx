  ...
  const updateVisibility = useCallback((newCount: number) => {
    ...
    setNodes((nds) =>
      nds.map((node) => {
        ...
        if (node.type === 'custom') {
          ...
          isVisible = newCount >= (noteAppearsMap.get(node.id) as number);
        }
        ...
        // ⚡ Bolt Optimization: Preserve existing static properties by spreading node
        return {
          ...node,
          style: {
            ...node.style,
    ...
    setEdges((eds) =>
      eds.map((edge) => {
        ...
        // ⚡ Bolt Optimization: Preserve existing static properties by spreading edge
        return {
          ...edge,
          label: expectedLabel,
          animated: isVisible,
