...
// ⚡ Bolt Optimization: Use functional state updates to preserve React Flow internal geometry during step changes
...
function App() {
  ...
  const updateVisibility = useCallback((newCount: number) => {
    setNodes((nds) => nds.map((node) => {
      ...
      const expectedPointerEvents = (isVisible ? 'auto' : 'none') as 'auto' | 'none';

      if (node.style?.opacity === targetOpacity && node.style?.pointerEvents === expectedPointerEvents) {
        return node;
      }
      return {
        ...node,
        style: { ...node.style, opacity: targetOpacity, pointerEvents: expectedPointerEvents },
      };
    }));
    ...
    setEdges((eds) => eds.map((edge) => {
      ...
      const targetOpacity = isVisible ? 1 : 0;
      const targetLabel = isVisible ? (edge.data?.originalLabel as string | undefined) : undefined;

      if (edge.style?.opacity === targetOpacity && edge.label === targetLabel && edge.animated === isVisible) {
        return edge;
      }
      return {
        ...edge,
