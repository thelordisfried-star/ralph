...
const stepDataMap = Object.fromEntries(allSteps.map(s => [s.id, {
  title: s.label,
  description: s.description,
  phase: s.phase,
}]));

const noteDataMap = Object.fromEntries(notes.map(n => [n.id, {
  content: n.content,
  color: n.color
}]));
...
function createNode(step: typeof allSteps[0], visible: boolean, position?: { x: number; y: number }): Node {
  return {
    ...
    style: visible ? visibleStyle : invisibleStyle,
  };
}
