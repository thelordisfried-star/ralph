...
// ⚡ Bolt Optimization: Replace O(N) recreations with functional setters to modify only target properties (style, label),
// preserving the referential stability of nodes and edges to maximize React.memo performance.
...
const updateEdgesVisibility = (count: number) => (eds: Edge[]) =>
  eds.map((edge) => {
    // Determine visibility based on source and target
    // We can extract source/target from edge.source and edge.target
