...
function App() {
  ...
  const initialNodes = useMemo<Node[]>(() => {
    ...
    const stepNodes = allSteps.map((step, index) =>
      createNode(step, index < 1, currentPositions[step.id])
    );
  ...
  const updateVisibility = useCallback((newCount: number) => {
    ...
    setNodes((nds) => nds.map((node) => {
      ...
      if (node.type === 'custom') {
        ...
        if (stepIndex !== undefined) visible = stepIndex < newCount;
      } else if (node.type === 'note') {
        const appearsWithStep = node.data.appearsWithStep as number | undefined;
        if (appearsWithStep !== undefined) visible = newCount >= appearsWithStep;
      }
