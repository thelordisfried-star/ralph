...
function App() {
  ...
  const updateVisibility = useCallback((newCount: number) => {
    setNodes((nds) => nds.map((node) => {
      ...
      if (node.type === 'custom') {
        ...
        const appearsWithStep = noteVisibilityMap.get(node.id) ?? 999;
    ...
    setEdges((eds) => eds.map((edge) => {
      ...
      if (edge.style?.opacity === opacity && edge.animated === visible) {
        return edge;
      }
  ...
  const handleReset = useCallback(() => {
    ...
    updateVisibility(1);
  }, [setNodes, setEdges, initialNodes, initialEdges, updateVisibility]);
