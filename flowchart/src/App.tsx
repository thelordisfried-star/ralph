  ...
  const handleNext = useCallback(() => {
    if (visibleCount < allSteps.length) {
      ...
      setNodes((nds) => nds.map((node) => {
        const isStep = stepIndexMap.has(node.id);
        const isNote = noteVisibilityMap.has(node.id);
        let visible = false;

        if (isStep) {
          visible = (stepIndexMap.get(node.id) ?? -1) < newCount;
        } else if (isNote) {
          visible = newCount >= (noteVisibilityMap.get(node.id) ?? 999);
        } else {
            return node;
        }
      ...
      setEdges((eds) => eds.map((edge) => {
        ...
        return {
          ...edge,
          animated: visible,
          label: visible ? edgeLabelMap.get(edge.id) as string | undefined : undefined,
          style: {
            ...edge.style,
            opacity: targetOpacity,
          },
