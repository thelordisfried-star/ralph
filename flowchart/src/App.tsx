...
function createNoteNode(note: typeof notes[0], visible: boolean, position?: { x: number; y: number }, appearsWithStep?: number): Node {
  return {
    id: note.id,
    type: 'note',
...
const getNodes = (count: number, currentPositions: { [key: string]: { x: number; y: number } }) => {
  ...
  const noteNodes = notes.map(note => {
    ...
    return createNoteNode(note, noteVisible, currentPositions[note.id], note.appearsWithStep);
  });
...
function App() {
  ...
  const initialNodes = useMemo<Node[]>(() => getNodes(1, { ...positions }), []);
  const initialEdges = useMemo<Edge[]>(() => edgeConnections.map((conn, index) =>
    createEdge(conn, index < 0, stepIndexMap.get(conn.source) ?? -1, stepIndexMap.get(conn.target) ?? -1)
  ), []);
  ...
  const updateGraphVisibility = useCallback((newCount: number) => {
    // ⚡ Bolt Optimization: Use functional state updates with embedded metadata to prevent O(N) recreations and object allocations.
    setNodes((nds) =>
      nds.map((node) => {
  ...
  const handleNext = useCallback(() => {
    if (visibleCount < allSteps.length) {
      ...
      updateGraphVisibility(newCount);
    }
  }, [visibleCount, updateGraphVisibility]);
