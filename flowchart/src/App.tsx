...
const noteAppearsWithMap = new Map(notes.map(n => [n.id, n.appearsWithStep]));
...
const getEdgeVisibility = (conn: { source: string; target: string }, visibleStepCount: number) => {
...
function App() {
  ...
  // ⚡ Bolt Optimization: Functional state updates to preserve referential equality and avoid recreating nodes on every step
  const updateVisibility = useCallback((newCount: number) => {
    setNodes((nds) => nds.map((node) => {
      let isVisible = false;
      if (node.type === 'note') {
        isVisible = newCount >= (noteAppearsWithMap.get(node.id) ?? 999);
      } else {
        isVisible = (stepIndexMap.get(node.id) ?? 999) < newCount;
      }
    ...
    setEdges((eds) => eds.map((edge) => {
      const isVisible = getEdgeVisibility({ source: edge.source, target: edge.target }, newCount);
      const targetOpacity = isVisible ? 1 : 0;

      if (edge.style?.opacity === targetOpacity) return edge;

      const originalLabel = edgeLabelMap.get(edge.id);

      return {
        ...edge,
        label: isVisible ? (originalLabel as string | undefined) : undefined,
        animated: isVisible,
        style: {
          ...edge.style,
          opacity: targetOpacity,
