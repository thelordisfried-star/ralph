...
function createNode(step: typeof allSteps[0], visible: boolean, index: number, position?: { x: number; y: number }): Node {
  return {
    id: step.id,
    type: 'custom',
...
function App() {
  ...
  const initialNodes = useMemo<Node[]>(() => {
    const stepNodes = allSteps.map((step, index) => createNode(step, index < 1, index, positions[step.id]));
    const noteNodes = notes.map(note => createNoteNode(note, 1 >= note.appearsWithStep, positions[note.id]));
    return [...stepNodes, ...noteNodes];
  }, []);

  const initialEdges = useMemo<Edge[]>(() => {
    const tempIndexMap = new Map(allSteps.map((s, i) => [s.id, i]));
    return edgeConnections.map((conn) => {
      const sourceIndex = tempIndexMap.get(conn.source) ?? -1;
      const targetIndex = tempIndexMap.get(conn.target) ?? -1;
  ...
  // ⚡ Bolt Optimization: Switched to functional state updates mapping over existing elements.
  // This preserves referential equality for nodes/edges whose visibility hasn't changed,
  // eliminating O(N) object allocations and unnecessary React Flow re-renders per click.
    ...
    if (visibleCount < allSteps.length) {
      ...
      setNodes(nds => nds.map(node => {
        ...
        if (node.type === 'custom') {
          ...
          visible = newCount >= (node.data.appearsWithStep as number);
        }
        ...
        return {
          ...node,
          style: { ...node.style, opacity, pointerEvents: pointerEvents as 'auto' | 'none' }
        };
      }));
      ...
      setEdges(eds => eds.map(edge => {
        ...
        const opacity = visible ? 1 : 0;
        const label = visible ? (edge.data.originalLabel as string | undefined) : undefined;
