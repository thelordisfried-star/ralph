...
function createNoteNode(note: typeof notes[0], visible: boolean, appearsWithStep: number, position?: { x: number; y: number }): Node {
  return {
    id: note.id,
    type: 'note',
    position: position || positions[note.id],
    data: { content: note.content, color: note.color, appearsWithStep },
    style: {
      opacity: visible ? 1 : 0,
      transition: 'opacity 0.5s ease-in-out',
      pointerEvents: visible ? 'auto' : 'none',
...
// ⚡ Bolt Optimization: Eliminated O(N) array recreation and ref usage on every navigation step.
// Initial state now fully embeds O(1) metadata for functional updates without recreating element lists.
...
function App() {
  ...
  const initialNodes = useMemo<Node[]>(() => {
    const stepNodes = allSteps.map((step, index) =>
      createNode(step, index < 1, index, positions[step.id])
    );
    const noteNodes = notes.map(note =>
      createNoteNode(note, 1 >= note.appearsWithStep, note.appearsWithStep, positions[note.id])
    );
  ...
  const initialEdges = useMemo<Edge[]>(() => edgeConnections.map(conn => {
    ...
    // Initial visible step count is 1 (index 0 is visible, index 1+ is hidden)
    const isVisible = sourceIndex < 1 && targetIndex < 1;
    return createEdge(conn, isVisible, sourceIndex, targetIndex);
  }), []);
  ...
  const handleNext = useCallback(() => {
    if (visibleCount < allSteps.length) {
      ...
      // ⚡ Bolt Optimization: Use functional state updates with referential equality preservation.
      // O(N) recreations and lookup arrays are replaced by O(1) embedded data checks.
      setNodes((nds) =>
        nds.map((node) => {
          ...
          if (node.style?.opacity === (isVisible ? 1 : 0)) {
            return node;
          }
      ...
      setEdges((eds) =>
        eds.map((edge) => {
          ...
          const isVisible =
            (edge.data.sourceIndex as number) < newCount &&
            (edge.data.targetIndex as number) < newCount;

          if (edge.style?.opacity === (isVisible ? 1 : 0)) {
            return edge;
          }

          return {
            ...edge,
            label: isVisible ? edge.data.originalLabel as string : undefined,
            animated: isVisible,
            style: {
              ...edge.style,
              opacity: isVisible ? 1 : 0,
