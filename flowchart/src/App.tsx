...
const edgeLabelMap = new Map(edgeConnections.map(c => [`${c.source}-${c.target}`, c.label]));
...
function App() {
  ...
  // ⚡ Bolt Optimization: Replace O(N) node/edge recreations with selective React functional state updates.
  // Extracted duplicate mapping logic into a DRY helper function.
  const updateVisibility = useCallback((newCount: number) => {
    ...
    setNodes(nds => nds.map(node => {
      ...
      if (isNote) {
        const appearsWithStep = noteVisibilityMap.get(node.id);
        isVisible = appearsWithStep !== undefined ? newCount >= appearsWithStep : false;
      } else {
      ...
      return {
        ...node,
        style: { ...node.style, opacity: targetOpacity, pointerEvents: (isVisible ? 'auto' : 'none') as 'auto' | 'none' }
      };
    }));
    ...
    setEdges(eds => eds.map(edge => {
      ...
      const sourceIndex = stepIndexMap.get(sourceNode) ?? -1;
      const targetIndex = stepIndexMap.get(targetNode) ?? -1;
      ...
      return {
        ...edge,
        animated: isVisible,
        label: isVisible ? edgeLabelMap.get(`${sourceNode}-${targetNode}`) : undefined,
