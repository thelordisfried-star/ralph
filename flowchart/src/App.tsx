...
function createNode(step: typeof allSteps[0], index: number, visible: boolean, position?: { x: number; y: number }): Node {
  return {
    id: step.id,
    type: 'custom',
...
function App() {
  ...
  const initialNodes = useMemo<Node[]>(() => {
    const stepNodes = allSteps.map((step, index) =>
      createNode(step, index, index < 1, positions[step.id])
    );
  ...
  const initialEdges = useMemo<Edge[]>(() => edgeConnections.map((conn) =>
    createEdge(conn, false) // Note: Initial visibility logic correctly embedded in createEdge via false for count=1
  ), []);
  ...
  const handleNext = useCallback(() => {
    if (visibleCount < allSteps.length) {
      ...
      setEdges(eds => eds.map(edge => {
        if (!edge.data || typeof edge.data.sourceIndex !== 'number' || typeof edge.data.targetIndex !== 'number') return edge;
        const visible = edge.data.sourceIndex < newCount && edge.data.targetIndex < newCount;
        const opacity = visible ? 1 : 0;
